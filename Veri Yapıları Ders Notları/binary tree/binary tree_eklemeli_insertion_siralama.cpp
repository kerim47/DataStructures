//Eklemeli Sýralama (Insertion Sort)
...
void insertionSort(node* agac) {
	...
	while (current != NULL) {
		node* temp = current->sag;

		// Sorted listeyi tarayarak current düðümünün yerini bulma
		if (sorted == NULL || current->data <= sorted->data) {
			...
			while (traverse->sag != NULL && current->data > traverse->sag->data) {
				traverse = traverse->sag;
			}
			current->sag = traverse->sag;
			traverse->sag = current;
		}

		current = temp;
	}

	// Sýralanmýþ düðümleri ekrana yazdýrma
	node* traverse = sorted;
